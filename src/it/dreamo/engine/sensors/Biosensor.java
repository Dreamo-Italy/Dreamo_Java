...
abstract class Biosensor
{
 ...
 //********* PUBLIC MEMBERS ***********
  ...
  {   
    ...
    sampleToExtract = PApplet.ceil ((GlobalParams.sampleRate/(GlobalParams.fps))); 
  ...
  public void checkCalibration() // set defaultValue to the initial condition of the sensor
  {
    if ( this.needCalibration() )
    {
      ...
      if ( calibrationCounter > parent.frameRate*CALIBRATION_TIME )
         endCalibration();        
  ...
  {
    if ( !calibrating )                    { PApplet.println("WARNING: not calibrating error");     return; }
    else if ( calibrated )                 { PApplet.println("WARNING: already calibrated error");  return; }
    else if ( incomingValues.size() == 0 ) { PApplet.println("ERROR: incomingValues is empty");     return; }

    calibrationValues.append( incomingValues );
 
    PApplet.println("Calibrating sensor: " + getID());
    PApplet.println();
  ...
  protected void endCalibration()
  {
    ...
    float average = computeAverage( calibrationValues , ( PApplet.min(calibrationValues.array() ) + PApplet.max(calibrationValues.array() ) )/2 );
    ...
    PApplet.println( "new normalized average: " + average );
  ...
  private float normalizeValue(float toNormalize) 
  {
      ...
      float normalized = parent.map ( toNormalize, getPhysicalMin(), getPhysicalMax(), 0, 1);
      return normalized;
  }
 ...
 // simple "average" function
  public float computeAverage(FloatList inputList, float oldAverage)
  {
    if ( inputList == null )
    {  
    	PApplet.println(" ERROR: computeAverage: NULL argument ");
       return oldAverage;
    }
    ...
    int listSize = inputList.size();
      if ( listSize == 0 ) 
        { PApplet.println("ERROR: Argument of computeAverage is an empty list. "); 
            return oldAverage; 
          }       
      else
      {
          ...
          for(int i=0; i < listSize ;i++)
            { 
              if ( inputList.get(i) < PApplet.MAX_FLOAT )
                 {
                   sum += inputList.get(i); 
                 }
              else
                 { 
                   ...
                   sum += inputList.get(i); 
                   PApplet.println("WARNING: corrupted float found during computeAverage. Index: "+i);
                 }
            }
  ...
  // Differentiator
    for (int i=0; i<a.length;i++){
      if(i>3){a[i]= 0.1f*(2*a[i] + a[i-1] -a[i-3]-2*a[i-4]);}
     }
    return a;
 }
   ...
   public float computeStdDev(float [] tacogramma,int b)
   {
     ...
     for (int i=0; i< N;i++)
     {
      sumSq=+PApplet.sq(a[i]);
     }
     xmsq=PApplet.sq(avarage);
     xisq=sumSq/N;
     StdDev=PApplet.sqrt(xisq-xmsq);
     maxAva=(1.9f-0.28f)/2f;
     maxDev=PApplet.sqrt(((PApplet.sq(1.9f)-PApplet.sq(0.48f))/2) -PApplet.sq(maxAva));
