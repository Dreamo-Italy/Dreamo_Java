...
import it.dreamo.engine.*;
import it.dreamo.engine.video.scenes.*;
...
public class Stage
{
	...
	{
		int i;
		scenesList = new Scene[GlobalParams.SCENES_MAX];
		scenesNumber = 0;
		currentScene = null;
		currentSceneIndex = -1;
		global_stage_change_cnt = 0;
		scene_score = new int[GlobalParams.SCENES_MAX];
    
    firstChange = false;
    
    for(i = 0; i < GlobalParams.SCENES_MAX; i ++) //<>// //<>// //<>//
    {
      scene_score[i] = 0;
    }
	}
  ...
  firstChange = toCopy.firstChange;
	...
	{
		if(scenesNumber < GlobalParams.SCENES_MAX)
		{
		...
		{
			PApplet.println("Warning: reached maximum number of scenes for the Stage. Last Scene wasn't added.");
		}
	}
	...
	public void nextScene()  
	{
		...
		{
			PApplet.println("Warning: there are no scenes in the stage.");
		}
	}
  ...
  { 
    ...
    scene_sel = (int)PApplet.round((q_gsr + q_ecg)/2);
    scene_score[scene_sel] = scene_score[scene_sel] + 1;
    
    if(global_stage_change_cnt == GlobalParams.CHANGE_CHECK)
    {
      max_sel = - 1;
      max = 0;
      ...
      {
        if(scene_score[i] > GlobalParams.SOGLIA_SEL )
        {  
	...
	public void popScene()
	{
		...
		{
			PApplet.println("Warning: cannot pop any more scenes from the Stage."); //<>// //<>//
		}
	}
