...
public class BioMood 
{
  ...
  public BioMood()
  {
    arousal = new float[GlobalParams.sensorNumber];
  }
  
  //copy constructor
  public BioMood(BioMood toCopy)
  {
    for(int i=0; i<GlobalParams.sensorNumber;i++)
  ...
  public void update()
  {
    ...
    PApplet.println("gsr arousal: "+arousal[0]);
    PApplet.println("ecg arousal: "+arousal[1]);
  ...
  private float computeVariationGsrArousal()
  {
    ...
    varAr = PApplet.map(varAr, 0.7f, 5, 0, 1);
  ...
  private float computeEcgBpm()
  {
    ...
    ecgbPM = PApplet.map(ecgbPM, 40, 120, 0, 1);
